...
(function($) {
	...
	DateTimePicker.prototype = {
		...
		init : function(element, options) {
			...
			this.widget = $(
					getTemplate(this.timeIcon, options.pickDate,
							...
							options.pickSeconds, options.collapse,this)).appendTo(
					"body");
		...
		show : function(e) {
			var iVal = this.$element.find("input").attr('id');
			if(getVal(iVal) == '') {
		...
		hide : function() {
			...
			this.showMode();
			//this.set();-->Modified to prevent auto selecting date
			this.$element.trigger({
				type : "hide",
				date : this._date
			});
		...
		_detachDatePickerEvents : function() {
			this.widget.off("click", ".datepicker *", this.widget.hide());
			this.widget.off("click", "[data-action]",this.widget.hide());
	...
	function getTemplate(timeIcon, pickDate, pickTime, is12Hours, showSeconds,
			collapse,widget) {
			var str = widget.$element.find("input").attr('id');
		if (pickDate && pickTime) {
			...
			return '<div class="bootstrap-datetimepicker-widget dropdown-menu">'
					...
					+ "<div style='width:100%; text-align:center;'><a href='javascript:calenderClearFun(&quot;"+str+"&quot;);' >Clear</a></div></div>";
		}
	}
