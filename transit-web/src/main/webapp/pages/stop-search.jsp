	...
	<div class="container-fluid cw-admin-search-widget">
		...
		<div class="col-xs-12 content-wrapper">
			...
			<div
				...
				<a href="stop-search">Search</a>
			</div>
			<div class="tab-header-container">
				<a href="stop-registration">Create</a>
			</div>
			...
			<div class="searchConteiner">
				<div class="row rowfluidalignment">
					<div class="col-sm-12">
						...
						<form action="stop-search-pagination" name="paginationForm"
							method="post">
							<input type="hidden" id="pageNumberId" name="pageNumber" />
						</form>
						<form action="edit-stop-action" name="editStopAction"
							method="post">
							<input type="hidden" id="stopEdit" name="stopId" />
						</form>

						<form action="stop-view" name="viewStopAction" method="post">
							<input type="hidden" id="stopView" name="stopId" />
						</form>

						<form:form action="downloadStopReport" name="downloadReport"
							method="post">
						...
						<form:form modelAttribute="stopSearchRequest" action="stop-search"
							method="post">
							...
							<div class="col-sm-12 widgetDescriptionForm">
								...
								<div class="row Overview-section">
									...
									<fieldset class="col-sm-3 form-group">
										<legend class="custom-legend">Stop Name</legend>
										<select id="stopId" path="stopId" name="stopId"
											class="form-control" type="text" value="" maxlength="50">
											<option value="">..:Select:..</option>
											<c:forEach items="${stopNameList}" var="stopNameList">
												<option value="${stopNameList.stopId}">${stopNameList.stopName}</option>
											</c:forEach>
										</select>
								...
								<div class="role-makerchecker-btn">
									<fieldset class="form-group">
										<div
											...
											<input type="submit" id="stop-search" value="Search"
												class="form-control button pull-right"> <a
												href="stop-search" class="form-control button pull-right"><legend class = "custom-legend-button">Reset</legend></a>
										</div>
									</fieldset>
								</div>
								<!-- Form Button End -->
								...
								<td colspan="9" class="searchRsltTblHeading">
									<h6>
										<label>Search Result</label> <span class="pull-right">Total
											Count : <label id="totalRecords">${stopDataSize}</label>
										</span>
									</h6>
								</td>
							</tr>
					...
					<table>
						...
						<table id="serviceResults"
							...
							<thead>
								...
								<tr role="row" class="tablesorter-headerRow">
									...
									<th data-column="0"
										...
										<div class="tablesorter-header-inner">Stop Id</div>
									</th>
									<th data-column="2"
										class="tablesorter-header tablesorter-headerUnSorted"
										tabindex="0" scope="col" role="columnheader"
									...
									<th data-column="2"
										...
										unselectable="on" aria-sort="none"
										aria-label="Type: No sort applied, activate to apply an ascending sort">
										<div class="tablesorter-header-inner">Stop Name</div>
									</th>
							...
							<tbody>
								...
								<c:if test="${ !(fn:length(stopListData) eq 0)}">
									<c:forEach items="${stopListData}" var="stopListData">
										<tr>
											<td>${stopListData.stopId}</td>
											<td>${stopListData.organizationName}</td>
											<td>${stopListData.ptoName}</td>
											<td>${stopListData.routeName}</td>
											<td>${stopListData.stageName}</td>
											<td>${stopListData.stopName}</td>

											<td><c:if test="${stopListData.status eq 'Active'}">
													<div class="active">Active</div>
												</c:if> <c:if test="${stopListData.status eq 'Suspended'}">
													<div class="suspended">Suspended</div>
												</c:if></td>
											<td><a
												href="javascript:viewStop('${stopListData.stopId}')"
												title="View"> <img src="../images/eyeimage.png"
													alt="eyeimage" title="view"></img></a> <c:choose>
													<c:when
														test="${fn:containsIgnoreCase(stopListData.status, 'Active') }">

														<a href="javascript:editStop('${stopListData.stopId}')"
															title="Edit"> <span
													...
													<c:when
														...
														<a class="operation-status operation-status_suspended"
															title="Suspend"
															href="javascript:changeStopStatus('${stopListData.stopId}','Suspended')"><img
															src="../images/deactive.png" alt="Suspend"
															title="Suspend"></img></a>
														<a class="operation-status operation-status_terminated"
															title="Terminate"
															href="javascript:changeStopStatus('${stopListData.stopId}','Terminated')"><i
															class="glyphicon glyphicon-trash" aria-hidden="true"></i></a>
													</c:when>
													<c:when
														test="${fn:containsIgnoreCase(stopListData.status, 'Suspended') }">
														<a class="operation-status operation-status_active"
															title="Active"
															href="javascript:changeStopStatus('${stopListData.stopId}','Active')"><img
								...
								<td colspan="15" class="tblPagination">
									<div class="exportResults">
										<table>
											<tr>
												<a
													href="javascript:downloadReport('${portalListPageNumber}', 'XLS', ${stopDataSize})">
													<button type="button" class="btn btn-default">
														<img src="../images/excel.png" alt="excel download image">
													</button>
												</a>
												<a
													href="javascript:downloadReport('${portalListPageNumber}', 'PDF', ${stopDataSize})">
													<button type="button" class="btn btn-default">
										...
										test="${ !(fn:length(stopListData) eq 0)}">
...
<div id="stopDiv" class="locatioin-list-popup">
	<span class="glyphicon glyphicon-remove pull-right"
		onclick="closePopup()"></span>
	<h5 id="statusId">Change Status</h5>
	<form:form action="stop-status-update" name="changeStopStatusAction"
		method="post">
		<input type="hidden" id="stopId" name="stopId" />
