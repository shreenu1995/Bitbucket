...
<title>Payment Gateway Search</title>
...
<script src="../js/paymentGateway.js"></script>
</head>
		...
		<div class="col-xs-12 content-wrapper">
			<div class="breadCrumb">
				<span class="breadcrumb-text">Interface</span> <span
					...
					class="breadcrumb-text">Payment Gateway</span> <span
					class="glyphicon glyphicon-play icon-font-size"></span> <span
					class="breadcrumb-text">Search</span>
			</div>
			<div
				class="tab-header-container active-background marginL40 tab-header-active">
				<a href="pg-onboarding-search">Search</a>
			</div>
			<div class="tab-header-container">
				<a href="showCreatePg">Create</a>
			</div>

			<div class="searchConteiner">
				<div class="row rowfluidalignment">
					<div class="col-sm-12">
						<form action="pg-search-pagination" name="paginationForm"
							method="post">
							<input type="hidden" id="pageNumberId" name="pageNumber" />
						</form>
						<!-- Page Form Start -->
						<form:form action="downloadPaymentGatewayReport" name="downloadReport"
							method="post">
						...
						<form:form modelAttribute="pgRequest"
							action="pg-onboarding-search" method="post">
									...
									<div class="widgetDescriptionRow">
										...
										<fieldset class="col-sm-3 form-group">
											<legend class="custom-legend">
												<span class="imp-star">Payment Gateway Name</span>
											</legend>
											<form:input id="pgName" name="pgName" path="pgName"
												class="form-control" type="text" maxlength="50"></form:input>
											<div class="discriptionErrorMsg">
												<span id="pgNameMsgDiv" class="red-color-error">&nbsp;</span>
											</div>
										</fieldset>
									</div>
									<c:choose>
								...
								<div class="role-makerchecker-btn">
									<fieldset class="form-group">
										<div
											...
											<input type="submit" id="pg-onboarding-search" value="Search"
												class="form-control button pull-right"> <a
												href="pg-onboarding-search"
			...
			<c:if test="${searchFlag eq true}">
				<div class="searchResultsTable">
								...
								<td colspan="9" class="searchRsltTblHeading">
									<h6>
										<label>Search Result</label> <span class="pull-right">Total
											Count : <label id="totalRecords">${pgDataSize}</label>
										</span>
									</h6>
								</td>
							</tr>
						...
						<table id="serviceResults"
							...
							<thead>
								...
								<th data-column="0"
									...
									<div class="tablesorter-header-inner">Payment Gateway
										Name</div>
								</th>
								<th data-column="1"
									class="tablesorter-header tablesorter-headerUnSorted"
							...
							<tbody>
								...
								<c:choose>
									<c:when test="${!(fn:length(pgData) eq 0) }">
										<c:forEach items="${pgData}" var="pgData">
											<tr>
												<%-- <td>${vehicleTypeData.vehicleTypeId}</td> --%>
												<td>${pgData.pgName}</td>
												<td>${pgData.ptoName}</td>
												<td>${pgData.serviceUrl}</td>
												<td><a
													class="operation-status operation-status_terminated"
													title="Terminate"
													href="javascript:changePgStatus('${pgData.pgId}','Terminated')"><i
									...
									<c:otherwise>
										<tr>
											<td colspan="15" style="color: red">No Pg data found</td>
										</tr>
									</c:otherwise>
								</c:choose>
							</tbody>
					...
					<table
						...
						<c:if test="${!(fn:length(pgData) eq 0) }">
							<tr>
								<td colspan="15" class="tblPagination">
									<div class="exportResults">
										<table>
											<tr>
												<a
													href="javascript:downloadReport('${portalListPageNumber}', 'XLS', ${pgDataSize})">
													<button type="button" class="btn btn-default">
														<img src="../images/excel.png" alt="excel download image">
													</button>
												</a>
												<a
													href="javascript:downloadReport('${portalListPageNumber}', 'PDF', ${pgDataSize})">
													<button type="button" class="btn btn-default">
									...
									</div> <!-- Pagination --> <c:if test="${ !(fn:length(pgData) eq 0)}">
...
<div id="pgDiv" class="locatioin-list-popup">
	<span class="glyphicon glyphicon-remove pull-right"
		onclick="closePopup()"></span>
	<h5 id="statusId">Change Status</h5>
	<form:form action="deletePg" name="deletePg" method="post">
