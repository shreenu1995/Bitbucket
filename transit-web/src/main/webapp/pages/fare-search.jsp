			...
			<div class="searchConteiner">
				<div class="row rowfluidalignment">
					<div class="col-sm-12">
						...
						<form action="fare-search-pagination" name="paginationForm"
							method="post">
							<input type="hidden" id="pageNumberId" name="pageNumber" />
						</form>

						<form action="view-fare-action" name="viewFareManagementAction"
							method="post">
							<input type="hidden" id="fareView" name="fareId" />
						</form>

						<form action="edit-Fare-action" name="editFareAction"
							method="post">
							<input type="hidden" id="FareEdit" name="fareId" />
						</form>

						<form:form action="downloadFareReport" name="downloadReport"
							method="post">
						...
						<form:form modelAttribute="fareSearchRequest" action="fare-search"
							method="post">
							...
							<div class="col-sm-12 widgetDescriptionForm">
								...
								<div class="row Overview-section">
									...
									<fieldset class="col-sm-3 form-group">
										<legend class="custom-legend">Fare Name</legend>
										<form:input id="fareName" name="fareName" path="fareName"
								...
								<div class="role-makerchecker-btn">
									<fieldset class="form-group">
										<div
											...
											<input type="submit" id="fare-search" value="Search"
												class="form-control button pull-right"> <a
												href="fare-search" class="form-control button pull-right"><legend class = "custom-legend-button">Reset</legend></a>
										</div>
									</fieldset>
								</div>
								<!-- Form Button End -->
								...
								<td colspan="9" class="searchRsltTblHeading">
									<h6>
										<label>Search Result</label> <span class="pull-right">Total
											Count : <label id="totalRecords">${fareDataSize}</label>
										</span>
									</h6>
								</td>
							</tr>
					...
					<table>
						...
						<table id="serviceResults"
							...
							<thead>
								...
								<tr role="row" class="tablesorter-headerRow">
									...
									<th data-column="1"
										...
										unselectable="on" aria-sort="none"
										aria-label="Type: No sort applied, activate to apply an ascending sort">
										<div class="tablesorter-header-inner">Fare Name</div>
									</th>
									<th data-column="2"
										class="tablesorter-header tablesorter-headerUnSorted"
										tabindex="0" scope="col" role="columnheader"
										...
										unselectable="on" aria-sort="none"
										aria-label="Type: No sort applied, activate to apply an ascending sort">
										<div class="tablesorter-header-inner">Fare Type</div>
									</th>
									<th data-column="3"
										class="tablesorter-header tablesorter-headerUnSorted"
										tabindex="0" scope="col" role="columnheader"
									...
									<th data-column="4"
										...
										<div class="tablesorter-header-inner">Fare Amount</div>
									</th>
							...
							<tbody>
								...
								<c:if test="${ !(fn:length(fareList) eq 0)}">
									<c:forEach items="${fareList}" var="fareList">
										<tr>
											<td>${fareList.organizationName}</td>
											<td>${fareList.ptoName}</td>
											<td>${fareList.fareName}</td>
											<td>${fareList.fareType}</td>
											<td>${fareList.difference}</td>
											<td>${fareList.fareAmount}</td>

											<td><c:if test="${fareList.status eq 'Active'}">
													<div class="active">Active</div>
												</c:if> <c:if test="${fareList.status eq 'Suspended'}">
													<div class="suspended">Suspended</div>
												</c:if></td>
											<td><a href="javascript:viewFare('${fareList.fareId}')"
												title="View"><img src="../images/eyeimage.png"
													alt="eyeimage" title="view"></img></a> <c:choose>
													<c:when
														test="${fn:containsIgnoreCase(fareList.status, 'Active') }">
														<a href="javascript:editFare('${fareList.fareId}')"
															title="Edit"> <span
													...
													<c:when
														...
														<a class="operation-status operation-status_suspended"
															title="Suspend"
															href="javascript:changeFareStatus('${fareList.fareId}','Suspended')"><img
															src="../images/deactive.png" alt="Suspend"
															title="Suspend"></img></a>
														<a class="operation-status operation-status_terminated"
															title="Terminate"
															href="javascript:changeFareStatus('${fareList.fareId}','Terminated')"><i
															class="glyphicon glyphicon-trash" aria-hidden="true"></i></a>

													</c:when>
													<c:when
														test="${fn:containsIgnoreCase(fareList.status, 'Suspended') }">
														<a class="operation-status operation-status_active"
															title="Active"
															href="javascript:changeFareStatus('${fareList.fareId}','Active')"><img
								...
								<td colspan="15" class="tblPagination">
									<div class="exportResults">
										<table>
											<tr>
												<a
													href="javascript:downloadReport('${portalListPageNumber}', 'XLS', ${fareDataSize})">
													<button type="button" class="btn btn-default">
														<img src="../images/excel.png" alt="excel download image">
													</button>
												</a>
												<a
													href="javascript:downloadReport('${portalListPageNumber}', 'PDF', ${fareDataSize})">
													<button type="button" class="btn btn-default">
...
<div id="fareDiv" class="locatioin-list-popup">
	<span class="glyphicon glyphicon-remove pull-right"
		onclick="closePopup()"></span>
	<h5 id="statusId">Change Status</h5>
	<form:form action="update-Fare-status" name="changeFareStatusAction"
		method="post">
		<input type="hidden" id="FareIdStatus" name="fareId" />
		<input type="hidden" id="FareStatus" name="status" />
