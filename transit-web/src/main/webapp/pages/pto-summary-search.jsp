		...
		<div class="col-xs-12 content-wrapper">
			<div class="breadCrumb">
				<span class="breadcrumb-text">Reports</span> <span
					...
					class="breadcrumb-text">PTO Summary Report</span> <span
					class="glyphicon glyphicon-play icon-font-size"></span> <span
			...
			<div
				...
				<a href="pto-summary-search">Search</a>
			</div>
			...
			<div class="searchConteiner">
				<div class="row rowfluidalignment">
					<div class="col-sm-12">
						...
						<!-- Page Form Start -->
						<form action="pagination-for-pto-summary-search"
							name="paginationForm" method="post">
							<input type="hidden" id="pageNumberId" name="pageNumber" />
						</form>

						<form:form action="downloadPtoSummaryDownloadReport"
						...
						<form:form modelAttribute="ptoSummaryRequest"
							action="pto-summary-search" method="post">
			...
			<c:if test="${searchFlag eq true}">
				<div class="searchResultsTable">
						...
						<table id="serviceResults"
							...
							<thead>
								...
								<th data-column="2"
									...
									unselectable="on" aria-sort="none"
									aria-label="Name: No sort applied, activate to apply an ascending sort">
									<div class="tablesorter-header-inner">No Of Routes</div>
								</th>
								<th data-column="3"
									class="tablesorter-header tablesorter-headerUnSorted"
									tabindex="0" scope="col" role="columnheader"
									...
									<div class="tablesorter-header-inner">Tickets Issued</div>
								</th>
								...
								<th data-column="4"
									...
									<div class="tablesorter-header-inner">Total Revenue</div>
								</th>
								</tr>
							</thead>
							<tbody>
								...
								<c:choose>
									<c:when test="${!(fn:length(ptoSummaryData) eq 0) }">
										<c:forEach items="${ptoSummaryData}" var="ptoSummaryData">
											<tr>
												<td>${ptoSummaryData.organizationName}</td>
												<td>${ptoSummaryData.ptoName}</td>
												<td>${ptoSummaryData.noOfRoutes}</td>
												<td>${ptoSummaryData.ticketsIssued}</td>
												<td>${ptoSummaryData.totalRevenue}</td>
											</tr>
										</c:forEach>
									</c:when>
									<c:otherwise>
										<tr>
											<td colspan="15" style="color: red">No PTO Summary data
												found</td>
										</tr>
									</c:otherwise>
								</c:choose>
					...
					<table
						...
						<c:if test="${!(fn:length(ptoSummaryData) eq 0) }">
							<tr>
								<td colspan="15" class="tblPagination">
									<div class="exportResults">
										...
										test="${ !(fn:length(ptoSummaryData) eq 0)}">
